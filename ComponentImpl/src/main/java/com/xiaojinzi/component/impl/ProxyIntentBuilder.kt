...
class ProxyIntentBuilder(
    private val bundleBuilder: IBundleBuilderImpl<ProxyIntentBuilder> = IBundleBuilderImpl(),
    private val uriBuilder: IURIBuilderImpl<ProxyIntentBuilder> = IURIBuilderImpl(),
) :
    ...
    private var proxyActivity: KClass<out Activity> = ProxyIntentAct::class

    fun proxyActivity(clazz: KClass<out Activity>): ProxyIntentBuilder {
        proxyActivity = clazz
        return this
    }
    ...
    fun buildProxyIntent(): Intent {
        val intent = Intent(getApplication(), proxyActivity.java)
        ...
        intent.putExtra(
            ProxyIntentAct.EXTRA_ROUTER_PROXY_INTENT_CATEGORIES,
            ArrayList(intentCategories)
        )
        return intent
    }
